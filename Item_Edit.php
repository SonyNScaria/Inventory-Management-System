...
<nav class="navbar navbar-inverse navbar-static-top">
  <div class="container">
	...
	<div class="container">
  <h2></h2>    
<form class="col-md-12 center-block" action="Item_Edit_Process.php" method="POST">  
	...
	while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) 
{
	...
	?>	
     <tbody>
		...
		<td><input type="text" class="form-control" name="<?php echo "description".$row['ID']; ?>" value="<?php echo $row['DESCRIPTION'] ?>"></td>
        <td><input type="text" class="form-control" name="<?php echo "unitweight".$row['ID']; ?>" value="<?php echo $row['UNIT_WEIGHT'] ?>" onkeypress='return event.charCode >= 46 && event.charCode <= 57'></td>
		<td><input type="text" class="form-control" name="<?php echo "unitprice".$row['ID']; ?>" value="<?php echo $row['UNIT_PRICE'] ?>" onkeypress='return event.charCode >= 46 && event.charCode <= 57'></td>
...
else
{
	...
	{ 
		...
		<td><input type="text" class="form-control" name="<?php echo "name".$row['ID']; ?>" value="<?php echo $row['NAME'] ?>"></td>
		<td><input type="text" class="form-control" name="<?php echo "description".$row['ID']; ?>" value="<?php echo $row['DESCRIPTION'] ?>"></td>
        <td><input type="text" class="form-control" name="<?php echo "unitweight".$row['ID']; ?>" value="<?php echo $row['UNIT_WEIGHT'] ?>"></td>
		<td><input type="text" class="form-control" name="<?php echo "unitprice".$row['ID']; ?>" value="<?php echo $row['UNIT_PRICE'] ?>"></td>
		<td><label class="control-label col-sm-2" for="email"><?php echo $row['UNITS_SOLD'] ?></label></td>
		<td><input type="hidden" class="form-control" name="<?php echo $row['ID']; ?>" value="<?php echo $row['ID'] ?>"></td>
		</tr>
		</tbody>
