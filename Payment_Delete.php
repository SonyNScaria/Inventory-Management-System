...
<nav class="navbar navbar-inverse navbar-static-top">
  <div class="container">
	...
	<div class="container">
  <h2></h2>    
<form class="col-md-12 center-block" action="Payment_Delete_Process.php" method="POST">  
	...
	while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) 
{
	...
	?>	
     <tbody>
		<tr>
		<td><input type="text" class="form-control" name="<?php echo "date".$row['PID']; ?>" value="<?php echo $row['DATE'] ?>" readonly></td>
		<td><textarea class="form-control" name="<?php echo "customer".$row['PID']; ?>" value="" readonly><?php echo $row['CUSTOMER'] ?></textarea></td>
        <td><input type="text" class="form-control" name="<?php echo "amount".$row['PID']; ?>" value="<?php echo $row['AMOUNT'] ?>" readonly></td>
		<td><input type="text" class="form-control" name="<?php echo "remark".$row['PID']; ?>" value="<?php echo $row['REMARK'] ?>" readonly></td>
